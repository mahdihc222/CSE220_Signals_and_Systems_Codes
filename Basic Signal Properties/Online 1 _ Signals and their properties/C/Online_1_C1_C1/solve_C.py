...
def even_odd_decompose(x: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    """
    ev = 0.5 * (x+time_reverse(x))
    od = 0.5 * (x-time_reverse(x))
    return ev,od
...
def main():
    ...
    # ax[0].plot(t,x,c='green',lw=2,label='x(t)')
    # ax[0].plot(t,xe,c='blue',lw=2,label = 'xe(t)')
    # ax[0].plot(t,xo,c='red',lw=2,label='xo(t)')
    # ax[0].grid(alpha=0.4)
    # ax[0].legend(fontsize=8)
    # ax[0].set_xlabel("Time")
    # ax[0].set_ylabel("Amplitude")

    # ax[1].plot(t,x,c='red',lw=2,label = 'x(t)')
    # ax[1].plot(t,xr,c='orange',lw=2,label='x(-t)')
