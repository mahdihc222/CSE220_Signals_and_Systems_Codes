...
def stem_plot(ax, n, x, label,c):
    """A nicer stem plot for discrete-time sequences."""
    markerline, stemlines, baseline = ax.stem(n, x, label=label, linefmt=c)
...
def main():
    ...
    stem_plot(ax1, n, x, "original x[n]","b")
    stem_plot(ax1, n, x_time, f"time shift by n0={n0}","r")
    stem_plot(ax1, n, x_phase_equiv, f"phase change by phi0={phi0_equiv:.3f}","g")
    ax1.legend()
    fig1.tight_layout()

    
    phi0 = 3  # an arbitrary phase change
    ...
    stem_plot(ax2, n, x_phase, f"phase change by phi0={phi0:.3f}",'b')
    stem_plot(ax2, n, x_time_best, f"best time shift k={best_k}",'r')
    ax2.legend()
    fig2.tight_layout()
    plt.show()
